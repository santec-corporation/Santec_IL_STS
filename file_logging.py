...
def save_reference_json_data(ilsts: sts.StsProcess, strfilename: str):
    ...
    with open(strfilename, 'w') as exportfile:
        json.dump(ilsts._reference_data_array, exportfile ) #No indents or newlines for this large file. If needed, then look at the CSV instead.
...
def save_reference_result_data(ilsts: sts.StsProcess, strfilename: str):
    ...
    wavelengthtable = ref_data_array[0]["rescaled_wavelength"] #all of the wavelengths are all the same for any slot and channel. So just get the first one.
